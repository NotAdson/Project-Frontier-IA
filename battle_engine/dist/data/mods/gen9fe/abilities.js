...
const Abilities = {
  ...
  openingact: {
    ...
    onPrepareHit(source, target, move) {
      ...
      if (move.priority > 0) {
        ...
        const vol = source.volatiles["openingact"];
  ...
  dyschronometria: {
    ...
    onAnyModifySpD(spd, attacker, defender, move) {
      ...
      for (const paradox of [
        ...
        if (defender.hasAbility(paradox)) {
          if ((defender?.volatiles["quarkdrive"] || defender?.volatiles["protosynthesis"] || defender?.volatiles[paradox])?.bestStat !== "spd") {
            return;
          }
          this.debug("Dyschronometria nullify");
          return this.chainModify([3151, 4096]);
  ...
  electromagneticveil: {
    ...
    onSetStatus(status, target, source, effect) {
      ...
      if (effect?.status && !this.heal(target.baseMaxhp / 4)) {
        this.add("-immune", target, "[from] ability: Electromagnetic Veil");
      }
      return false;
    },
  ...
  pillage: {
    ...
    onUpdate(pokemon) {
      ...
      if (!this.runEvent("SetAbility", target, pokemon, this.effect, pillageAbil) || !this.runEvent("SetAbility", pokemon, pokemon, this.effect, ability)) return;
  ...
  ultraface: {
    ...
    onWeatherChange(pokemon, source, sourceEffect) {
      if (sourceEffect?.suppressWeather || !pokemon.hp) return;
  ...
  swordofrejuvenation: {
    ...
    onStart(pokemon) {
      ...
      this.add("-message", `${pokemon.name}'s Sword of Ruin weakened the Defense of all surrounding Pok\xE9mon!`);
    },
  ...
  unidentifiedflyingobject: {
    ...
    onTryHit(target, source, move) {
      if (target !== source && move.type === "Ground" && !source.volatiles["confusion"] && !(move.ignoreImmunity && (move.ignoreImmunity === true || move.ignoreImmunity["Ground"])) && target.isGrounded() === null) {
