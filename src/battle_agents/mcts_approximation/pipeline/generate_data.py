...
def run_simulation(args):
    ...
    try:
        ...
        if use_cheating_mcts:
            # MCTSAgent does not need a Neural Network model. It peeks at opponent moves.
            agent_p1 = MCTSAgent(problem, iterations=mcts_iterations)
            agent_p2 = MCTSAgent(problem, iterations=mcts_iterations)
        else:
            # MCTSApproximationAgent loads the Neural Network.
...
def generate_dataset(num_games=1000, processes=None, output_dir="data/games", mcts_iterations=100, use_cheating_mcts=False):
    ...
    # Path(__file__) resolves to src/battle_agents/mcts_approximation/pipeline/generate_data.py
    # .parents[4] takes us up to the project root (Pokemon/)
    engine_path = str(Path(__file__).resolve().parents[4] / "engine")
    
    # MCTS ITERATIONS:
    args = [(engine_path, "gen3randombattle", mcts_iterations, use_cheating_mcts) for _ in range(remaining_games)]
