"""
Trains the Neural Network for MCTSApproximationAgent with Auxiliary next-state heads.
...
def _parse_steps_aux(game_data):
    X, y_value, y_policy, X_next = [], [], [], []
    total_len = len(game_data)
    if total_len == 0:
        return X, y_value, y_policy, X_next
...
def load_data_from_files(files):
    ...
    for f in files:
        try:
            ...
            gx, gv, gp, gn = _parse_steps_aux(game_data)
            X += gx
            y_value += gv
            y_policy += gp
            X_next += gn
...
def build_model(num_dense: int, num_moves: int, num_species: int,
    ...
    """Builds the multi-input value+policy network with auxiliary heads."""
...
def export_to_onnx(model, onnx_path, num_species):
    ...
    try:
        ...
        if keras.backend.backend() == "torch":
            ...
            dummy_inputs = (
                torch.zeros((1, NUM_DENSE_FEATURES), dtype=torch.float32),
                torch.zeros((1, 12), dtype=torch.int32),
                torch.zeros((1, 48), dtype=torch.int32),
                torch.zeros((1, 12), dtype=torch.int32),
                torch.zeros((1, 12), dtype=torch.int32),
            )
            ...
            torch.onnx.export(
                ...
                output_names=[
                    "value", "policy",
                    "aux_field", "aux_own_hp", "aux_opp_hp",
                    "aux_own_statuses", "aux_opp_statuses",
                    "aux_own_boosts", "aux_opp_boosts",
                    "aux_own_stats", "aux_opp_stats",
                    "aux_own_types", "aux_opp_types",
...
def train(data_dir: str = "data", model_save_path: str = "data/mcts_model.keras", max_games_buffer: int = 2500, epochs: int = 15):
    ...
    print("Loading training data (with next-state parsing)...")
    X_train, y_value_train, y_policy_train, X_next_train = load_data_from_files(train_files)
    print("Loading validation data (with next-state parsing)...")
    X_val, y_value_val, y_policy_val, X_next_val = load_data_from_files(val_files)
    ...
    # Shuffle turns
    def _shuffle(X, yv, yp, Xn):
        idx = np.random.permutation(len(X))
        return X[idx], yv[idx], yp[idx], Xn[idx]

    X_train, y_value_train, y_policy_train, X_next_train = _shuffle(X_train, y_value_train, y_policy_train, X_next_train)
    if len(X_val) > 0:
        X_val, y_value_val, y_policy_val, X_next_val = _shuffle(X_val, y_value_val, y_policy_val, X_next_val)
    ...
    # Extract target dictionary for auxiliary outputs
    y_aux_train = extract_aux_targets_batch(X_next_train, num_species)
    train_targets = {"value": y_value_train, "policy": y_policy_train}
    train_targets.update(y_aux_train)

    val_data = None
    if len(X_val) > 0:
        X_dense_val, X_species_val, X_moves_val, X_items_val, X_abilities_val = _split_features(X_val)
        y_aux_val = extract_aux_targets_batch(X_next_val, num_species)
        val_targets = {"value": y_value_val, "policy": y_policy_val}
        val_targets.update(y_aux_val)
    ...
    if os.path.exists(model_save_path):
        ...
        try:
            # Try to load directly (if model was already built/saved with aux heads and deserialization succeeds)
            ...
            print(f"Direct model load failed or mismatched architecture: {e}. Building new model with aux heads and loading weights directly...")
    ...
    export_to_onnx(model, onnx_path, num_species)
