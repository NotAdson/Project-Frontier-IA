...
def build_model(num_dense: int, num_moves: int, num_species: int,
    ...
    """Builds the multi-input value+policy network (8 inputs, 2 outputs)."""

    # Inputs — own side
    inp_dense       = keras.layers.Input(shape=(num_dense,), name="dense_features")
    inp_species     = keras.layers.Input(shape=(6,),  name="species_indices",    dtype="int32")
    inp_items       = keras.layers.Input(shape=(6,),  name="item_indices",       dtype="int32")
    inp_abilities   = keras.layers.Input(shape=(6,),  name="ability_indices",    dtype="int32")
    inp_bench_moves = keras.layers.Input(shape=(24,), name="bench_move_indices", dtype="int32")
    inp_moves       = keras.layers.Input(shape=(4,),  name="move_indices",       dtype="int32")
    # Inputs — opponent (public only)
    inp_opp_species = keras.layers.Input(shape=(6,),  name="opp_species_indices", dtype="int32")
    inp_opp_moves   = keras.layers.Input(shape=(24,), name="opp_move_indices",    dtype="int32")

    # Embedding branches — own side
    emb_species     = keras.layers.Flatten()(keras.layers.Embedding(num_species,  16, name="emb_species")(inp_species))
    emb_items       = keras.layers.Flatten()(keras.layers.Embedding(num_items,     8, name="emb_items")(inp_items))
    emb_abilities   = keras.layers.Flatten()(keras.layers.Embedding(num_abilities, 8, name="emb_abilities")(inp_abilities))
    emb_bench_moves = keras.layers.Flatten()(keras.layers.Embedding(num_moves,    16, name="emb_bench_moves")(inp_bench_moves))
    emb_moves       = keras.layers.Flatten()(keras.layers.Embedding(num_moves,    16, name="emb_moves")(inp_moves))
    # Embedding branches — opponent (separate weights, shared vocab sizes)
    emb_opp_species = keras.layers.Flatten()(keras.layers.Embedding(num_species,  16, name="emb_opp_species")(inp_opp_species))
    emb_opp_moves   = keras.layers.Flatten()(keras.layers.Embedding(num_moves,    16, name="emb_opp_moves")(inp_opp_moves))

    concat = keras.layers.Concatenate()(
        [inp_dense, emb_species, emb_items, emb_abilities,
         emb_bench_moves, emb_moves, emb_opp_species, emb_opp_moves]
    )
...
def train(data_dir: str = "data", model_save_path: str = "data/mcts_model.keras", max_games_buffer: int = 2500):
    ...
    # Split into 8 named inputs
    splits_train = _split_features(X_train)
    X_dense_train, X_species_train, X_items_train, X_abilities_train, X_bench_moves_train, X_moves_train, X_opp_species_train, X_opp_moves_train = splits_train
